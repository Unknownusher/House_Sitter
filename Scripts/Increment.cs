...
public class Increment : MonoBehaviour {
    ...
    float visibilityDuration = 3f;

    float peepsTimer = 0f;
    float peepsDeathTimer = 0f;

    float peepsDeathDuration = 4f;
    float peepsDuration = 3f;

    float crooketTimer = 0f;
    float crooketDuration = 5f;

    float RatSoarTimer = 0f;
    float RatSoarDuration = 1f;

    float clydeTimer = 0f;
    float clydeFlashTimer = 0f;
    float clydeDuration = 6f;
    ...
    IEnumerator CountTime() {
        ...
        while (!GameWon) {
            ...
            if (hours < 12 && hours != 6 && isJeremyActivated == false) { //if the hour is 1am or further and jeremy isn't activated yet
            ...
            if (((hours == 12 && tens >= 4) || hours < 12) && hours != 6 && isPeepsActivated == false) //12:40 or later
            {
            ...
            if (hours == 12 && tens < 4 && hours != 6 && !phoneSource.isPlaying)
                tens = 4;

            if (hours < 12 && tens >= 3 && hours != 6 && isCrooketActivated == false) { //1:30
            ...
            if (hours < 12 && hours >= 2 && hours != 6 && isRatSoarActivated == false) { //2:00
            ...
            if (hours < 12 && hours >= 2 && tens >= 4 && second >= 5 && hours != 6 && isClydeActivated == false) //2:45
            {
    ...
    IEnumerator JeremyJumpscare() {
        ...
        yield return new WaitForSeconds(2f);
        SceneManager.LoadScene("Jeremy_GameOverScene");
    ...
    IEnumerator PeepsJumpscare() {
        ...
        yield return new WaitForSeconds(2f);
        SceneManager.LoadScene("Peeps_GameOverScene");
    ...
    IEnumerator CrooketJumpscare() {
        ...
        yield return new WaitForSeconds(2f);
        SceneManager.LoadScene("Crooket_GameOverScene");
    ...
    IEnumerator RatSoar() {
        ...
        while (!GameWon) {
            ...
            yield return new WaitForSeconds(UnityEngine.Random.Range(50, 60));
    ...
    IEnumerator RatSoarJumpscare() {
        ...
        SceneManager.LoadScene("RatSoar_GameOverScene");
    ...
    IEnumerator Clyde() {
        ...
        while (!GameWon) {
            ...
            yield return new WaitForSeconds(UnityEngine.Random.Range(10f, 70f));
    ...
    IEnumerator ClydeJumpscare() {
        ...
        yield return new WaitForSeconds(2f);
        SceneManager.LoadScene("Clyde_GameOverScene");
